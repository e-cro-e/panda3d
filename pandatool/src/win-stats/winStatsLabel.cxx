...
set_pos(int x, int y, int width) {
  if (x != _x || y != _y || width != _width) {
    ...
    if (_tooltip_text == _text) {
      // If it doesn't fit, create a tooltip.
      if (_width < _ideal_width) {
        if (!_tooltip_window) {
...
update_text(bool use_fullname) {
  ...
  if (_window) {
    ...
    if (!_tooltip_text.empty() && (_tooltip_text != _text || _width < _ideal_width)) {
      if (!_tooltip_window) {
        create_tooltip_window();
      }
    /*} else {
...
/**
 * Creates the tooltip window for this label.
 */
void WinStatsLabel::
create_tooltip_window() {
  if (!_window || _tooltip_window) {
    return;
  }
  ...
  HWND parent = _graph->get_window();

  _tooltip_window = CreateWindowEx(0, TOOLTIPS_CLASS, nullptr,
                                   WS_POPUP,
...
/**
 * Destroys the tooltip window for this label.
 */
void WinStatsLabel::
destroy_tooltip_window() {
  if (!_tooltip_window) {
  ...
  info.hwnd = _window;
  info.uId = (UINT_PTR)_window;
  SendMessage(_tooltip_window, TTM_DELTOOL, 0, (LPARAM)&info);
